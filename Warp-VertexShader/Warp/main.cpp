...
void MouseFunc(int button, int state, int x, int y) 
{
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
		...
		{
			mouse_start.y = float(((window_height - y)/(0.2*window_height) - window_height*0.1) + (57.5/610)*window_height);
		}
	...
	{
		mouse.y = float(((window_height - y)/(0.2*window_height) - window_height*0.1) + (57.5/610)*window_height);
	}
